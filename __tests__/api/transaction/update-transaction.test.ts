...
import { assetInput, buyTransactionInput, sellTransactionInput, signUpInput, transactionUpdateInput } from "../../data";
...
describe(`${transactionPath} routes`, () => {
    ...
    describe(`${HttpMethod.PUT} ${transactionPath}${ApiPath.ID}`, () => {
        ...
        //     const { id, quantity, pricePerCoin } = response.body;

        //     expect(response.status).toBe(HttpCode.OK);
        //     expect(id.length).toBe(OBJECT_ID_LENGTH);
        //     expect({ quantity, pricePerCoin }).toEqual(transactionUpdateInput);
        ...
        it(`should return deleted ${TransactionType.SELL}-transaction`, async () => {
            ...
            expect(response.status).toBe(HttpCode.OK);
            expect(id.length).toBe(OBJECT_ID_LENGTH);
            expect({ quantity: -quantity, pricePerCoin }).toEqual(transactionUpdateInput);
            expect(updatedAsset.holdings).toBe(quantity);
            expect(updatedAsset.invested).toBe(quantity * pricePerCoin);
        });
